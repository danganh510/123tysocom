...
class LanguageController extends ControllerBase
{
    ...
    {
        $data = array('language_id' => -1,'language_active' => 'Y', 'language_order' => 1, 'language_country_code' => '' );
        if($this->request->isPost()) {
            ...
            if (count($messages) == 0) {
                ...
                if ($result === true) {
                    ...
                    $old_data = array();
                    $new_data = $data;
                    $data_log = json_encode(array('content_language' => array($new_language->getLanguageId() => array($old_data, $new_data))));
        ...
        $select_country = CountryGeneral::getCountryCombobox($data['language_country_code']);
    ...
    public function editAction()
    {
        ...
        if($this->request->isPost()) {
            ...
            if (count($messages) == 0) {
                ...
                if ($result === true) {
                    $old_data = $model_data;
                    $new_data = $input_data;
                    $data_log = json_encode(array('content_language' => array($id => array($old_data, $new_data))));
        ...
        $select_country = CountryGeneral::getCountryCombobox($input_data['language_country_code']);
        $messages["status"] = "border-red";
        $this->view->setVars([
            'formData' => $input_data,
            'messages' => $messages,
            ...
            foreach ($language_checked as $id) {
                ...
                if ($language_item) {
                    ...
                    if ($language_item->delete() === false) {
                        ...
                        $content_log[$id] = $old_data;
                    }
                }
            }
            if(count($content_log) > 0) {
                $message_delete = 'Delete '. count($content_log) .' Language successfully.';
                $msg_result['status'] = 'success';
                $msg_result['msg'] = $message_delete;
                $message = '';
                $data_log = json_encode(array('content_language' => $content_log));
                $activity = new Activity();
                $activity->logActivity($this->controllerName, $this->actionName, $this->auth['id'], $message, $data_log);
            }
            $this->session->set('msg_result', $msg_result);
