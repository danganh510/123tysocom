...
class Paginator
{
     ...
     * @return string
     */
    public function toHtmlFrontend($prev='<', $next='>')
    {
        if ($this->numPages <= 1) {
            return '';
        }
        ...
        if ($this->getPrevUrl()) {
            $html .= '<li><a href="' . htmlspecialchars($this->getPrevUrl()) . '"><strong>'. $prev .'</strong></a></li>';
        }

        foreach ($this->getPages() as $page) {
            if ($page['url']) {
                $html .= '<li' . ($page['isCurrent'] ? ' class="active"' : '') . '><a href="' . htmlspecialchars($page['url']) . '"><strong>' . htmlspecialchars($page['num']) . '</strong></a></li>';
            } else {
                $html .= '<li class="disabled"><span>' . htmlspecialchars($page['num']) . '</span></li>';
            }
        }
        ...
        if ($this->getNextUrl()) {
            $html .= '<li><a href="' . htmlspecialchars($this->getNextUrl()) . '"><strong>'. $next .'</strong></a></li>';
        }
        $html .= '</ul>';
        $html .= '</div>';
        $html .= '</div>';
